...
void receive_value()
{
    ...
    while ( more ) {
        ...
        /* Read one byte from UART and return the number of bytes read (or an
         * error status if no bytes were read) */
        uart_status = LOG_UART.Receive(&c, 1);
